...
@interface CZSchoolStarShopDetailCell()<UIScrollViewDelegate>

@end
@implementation CZSchoolStarShopDetailCell
...
- (void)setSportUserId:(NSString *)sportUserId{
    ...
    if (sportUserId) {
        ...
        if (!self.commentVC) {
            self.commentVC = [[CZSchoolStarShopCommentVC alloc]init];
        ...
        if (!self.caseVC) {
            self.caseVC = [[CZSchoolStarShopCaseVC alloc]init];
