...
@interface CZOrganizerDynamicVC ()
@property (nonatomic ,strong)UILabel *titleLab;//标题
@property (nonatomic ,strong)UIButton *backBtn;//返回按钮
@property (nonatomic ,strong)UIButton *shareBtn;//分享按钮
@property (nonatomic ,strong) CZOrganizerDynamicTableView *tableView;
@property (nonatomic ,assign) CGFloat alpha;
@end

@implementation CZOrganizerDynamicVC
    ...
    [self.tableView setScrollContentSize:^(CGFloat offsetY) {
        ...
        if (offsetY < 0) {
            ...
            if (!weakSelf.model.introduceOpen) {
                rect.size.height = ScreenScale(630) - offsetY;
            }else{
                CGFloat height = weakSelf.model.introduceHeight-weakSelf.model.singleHeight*2;
                rect.size.height = ScreenScale(630)+height - offsetY;
            }
...
- (CZOrganizerDynamicTableView *)tableView{
    if (!_tableView) {
        _tableView = [[CZOrganizerDynamicTableView alloc]initWithFrame:CGRectZero style:UITableViewStylePlain];
    }
    return _tableView;
}
