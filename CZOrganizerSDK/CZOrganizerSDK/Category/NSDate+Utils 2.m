...
+ (NSDate *)returnDayDawn:(NSString *)day {
    ...
    return [NSDate dateWithYear:year month:month day:day2 hour:0 minute:0 second:0];
}
...
+ (NSDate *)returnDay:(NSInteger)number {
    NSDate *yesterday = [NSDate dateWithTimeIntervalSinceNow:-number*(24*60*60)];
...
+ (NSDate *)returnWeekday {
    ...
    return [NSDate dateWithYear:year month:month day:Monday hour:0 minute:0 second:0];
}

+ (NSDate *)returnMonthday {
    NSDate *now = [NSDate date];
...
+ (NSDate *)returnMonth:(NSInteger)number {
...
-(NSString *)stringLoacalDateWithFormat:(NSString *)string {
    NSDateFormatter *format = [[NSDateFormatter alloc] init];
    [format setTimeZone:[NSTimeZone timeZoneWithAbbreviation:@"UTC"]];
