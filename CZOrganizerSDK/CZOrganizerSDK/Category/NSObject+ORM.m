...
static const char *kCMPropertyListKey = "CMPropertyListKey";
...
+ (instancetype)modelWithDict:(NSDictionary *)dict
{
    ...
    [dict enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull obj, BOOL * _Nonnull stop) {
        ...
        if ([propertyList containsObject:key]) {
            ...
            // 判断下value,是不是字典
            if ([obj isKindOfClass:NSClassFromString(@"__NSCFDictionary")] || [obj isKindOfClass:[NSDictionary class]]) { //  是字典对象,并且属性名对应类型是自定义类型
                // value:user字典 -> User模型
                // 获取模型(user)类对象
                NSString *ivarType = [[self class] dictWithModelClass][key];
                Class modalClass = NSClassFromString(ivarType);
                
                // 字典转模型
                if (modalClass) {
                    // 字典转模型 user
                    obj = [modalClass modelWithDict:obj];
                }
