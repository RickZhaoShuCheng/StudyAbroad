...
- (void)setItems:(NSArray *)items selectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        ...
        SPItem *selectedButton;
        if (selectedItemIndex == -1) {
            selectedButton = [[SPItem alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
        }else{
            selectedButton = [self.buttons objectAtIndex:selectedItemIndex];
        }
...
- (void)insertItemWithTitle:(NSString *)title atIndex:(NSInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
...
- (void)insertItemWithImage:(UIImage *)image atIndex:(NSInteger)itemIndex animated:(BOOL)animated {
    self.insert = YES;
    if (itemIndex > self.items.count) {return;}
...
- (void)removeItemAtIndex:(NSInteger)itemIndex animated:(BOOL)animated {
    if (itemIndex > self.items.count) {return;}
    // 被删除的按钮之后的按钮需要修改tag值
...
- (NSString *)titleForItemAtIndex:(NSInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setImage:(UIImage *)image forItemAtIndex:(NSInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (UIImage *)imageForItemAtIndex:(NSInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setTitle:(NSString *)title image:(UIImage *)image imagePosition:(SPItemImagePosition)imagePosition imageRatio:(CGFloat)ratio forItemIndex:(NSInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
        [button setTitle:title forState:UIControlStateNormal];
        [button setImage:image forState:UIControlStateNormal];
...
- (void)setEnabled:(BOOL)enaled forItemAtIndex:(NSInteger)itemIndex {
    if (itemIndex < self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (BOOL)enabledForItemAtIndex:(NSInteger)itemIndex {
    if (self.buttons.count) {
        SPItem *button = [self.buttons objectAtIndex:itemIndex];
...
- (void)setWidth:(CGFloat)width forItemAtIndex:(NSInteger)itemIndex {
    [self.setupWidths setObject:@(width) forKey:[NSString stringWithFormat:@"%zd",itemIndex]];
}

- (CGFloat)widthForItemAtIndex:(NSInteger)itemIndex {
...
//        [UIView animateWithDuration:0.5 animations:^{//动画太丑 2018年08月06日17:10:18 谢朋远
            [self setNeedsLayout];
            [self layoutIfNeeded];
//        }];
    }
...
//    dividingLine.backgroundColor = App_Line_Nomal_Color;
...
// 执行代理方法
- (void)delegatePerformMethodWithFromIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex {
...
- (void)setSelectedItemIndex:(NSInteger)selectedItemIndex {
    ...
    if (self.buttons.count) {
        SPItem *button;
        if (selectedItemIndex == -1) {
            button = [[SPItem alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
        }else{
            button = [self.buttons objectAtIndex:selectedItemIndex];
        }
        [self buttonInPageMenuClicked:button];
    }
}
...
- (void)setDelegate:(id<SPPageMenuDelegate>)delegate {
    ...
    if (self.buttons.count) {
        SPItem *button;
        if (_selectedItemIndex == -100) {
            button = [[SPItem alloc]initWithFrame:CGRectMake(0, 0, 0, 0)];
            button.tag = -100;
        }else{
            button = [self.buttons objectAtIndex:_selectedItemIndex];
        }
