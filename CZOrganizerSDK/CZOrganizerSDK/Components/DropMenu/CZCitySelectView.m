...
@interface CZCitySelectView ()<UITableViewDelegate, UITableViewDataSource>
{
@private
    /** 保存 选择的数据(行数) */
    NSInteger selects[3];
...
@implementation CZCitySelectView
...
- (instancetype)initWithAction:(MenuAction *)action {
    if (self = [super init]) {
        ...
        if (!action) {
            MenuAction *menuAction = [MenuAction actionWithTitle:@"" style:MenuActionTypeList];
            menuAction.ListDataSource = [NSMutableArray arrayWithArray:[CZCountryUtil sharedInstance].datas];
            for (CZSCountryModel *model in menuAction.ListDataSource) {
                model.dataSource = model.relatedArray;
                model.displayText = model.country.area_name;
                for (CZSCountryModel *modelIn in model.relatedArray) {
                    modelIn.dataSource = modelIn.relatedArray;
                    modelIn.displayText = modelIn.country.area_name;
                    for (CZSCountryModel *modelIn1 in modelIn.relatedArray) {
                        modelIn1.dataSource = modelIn1.relatedArray;
                        modelIn1.displayText = modelIn1.country.area_name;
                    }
                }
            }
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if (tableView == self.tableViewArr[0]) {
        ...
        CZSCountryModel *firstModel =self.action.ListDataSource[self.firstSelectRow];
        self.twoListDataSource = firstModel.dataSource;
        return firstModel.dataSource.count;
    }else {
        
        CZSCountryModel *firstModel =self.action.ListDataSource[self.firstSelectRow];
        CZSCountryModel *secondModel = firstModel.dataSource[self.secondSelectRow];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == self.tableViewArr[0]) {
        ...
        CZSCountryModel *selectModel = self.oneListDataSource[indexPath.row];
        ...
        CZSCountryModel *selectModel = self.twoListDataSource[indexPath.row];
        // 点击第二层时保存第一层选中的。
        if (!selectModel.dataSource.count) {
        ...
        CZSCountryModel *selectModel = self.threeListDataSource[indexPath.row];
        if (indexPath.row == 0 ) {
            // 如果选中的是0， 则将上一级选中的数据返回
...
-(void)adjustTableViewsWithCount:(int)count{
    ...
    for (int i = 0; i < count; i++) {
        ...
        adjustFrame.size.height = adjustFrame.size.height;
...
- (void)resetActionTitle:(CZSCountryModel *)selectModel selectRow:(NSInteger)row {
    // 表明只有一层数据。。选中第一行则恢复初始的title. 反正显示选中的数据
//    if(row == 0) {
...
- (void)resetOldSelectWithDataSource:(NSArray *)dataSource selectRow:(NSInteger) row {
    [dataSource enumerateObjectsUsingBlock:^(CZSCountryModel *model, NSUInteger idx, BOOL * _Nonnull stop) {
        model.seleceted = NO;
    }];
    CZSCountryModel *selectModel = dataSource[row];
