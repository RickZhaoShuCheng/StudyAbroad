...
{
    if (1 == _imagePathsGroup.count) return;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    // 传递图片圆角值
    cell.imgCornerRadius = _imgCornerRadius;
...
//            [KTTUtility setImageForImageView:cell.imageView Url:imagePath placeholderImage:self.placeholderImage];
    ...
    if (_titlesGroup.count && itemIndex < _titlesGroup.count) {
        if ([_titlesGroup[itemIndex] isKindOfClass:[NSAttributedString class]]) {
            cell.attributedTitle = _titlesGroup[itemIndex];
        }else{
